...
with torch.no_grad():  # batch size is len(dataset_valid)
    print(config.intents[torch.argmax(model(torch.unsqueeze(input1,0))).item()])
    print(config.intents[torch.argmax(model(torch.unsqueeze(input1_1,0))).item()])
    print(config.intents[torch.argmax(model(torch.unsqueeze(input2,0))).item()])
    print(config.intents[torch.argmax(model(torch.unsqueeze(input3,0))).item()])
    print(config.intents[torch.argmax(model(torch.unsqueeze(input4,0))).item()])
