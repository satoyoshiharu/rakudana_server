...
FACILITY = ['郵便局','ポスト','交番','区役所','市役所',
            'コンビニ','コンビニエンスストア','薬屋','ドラッグストア','ホームセンター',
            'レストラン','ラーメン','寿司','蕎麦屋','八百屋','魚屋','肉屋','花屋','青果店',
            'ガソリンスタンド','駐車場','ホテル','旅館','公園','駅']
QUALIFIER = ['近くの','このあたりの','この辺の','そばの','','','','','']
LOOKFOR_INTENT = ['はどこ','どこ','はどこにある','どこにある','を探して','探して','を見つけて','見つけて',
                  'の場所は','場所','の場所はどこ']
OBJ = ['地図','マップ']
...
if __name__ == "__main__":
    with open('../texts/map.txt', 'w') as f:
        for i in range(1000):
            ...
            f.write(f'{random.choice(MOOD)}{PLACE}{random.choice(LOOKFOR_INTENT)}\n')
